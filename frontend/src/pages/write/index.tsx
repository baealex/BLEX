...
export const getServerSideProps: GetServerSideProps = async ({ req }) => {
    ...
    return {
        props: {
            ...
            hasConnectedOpenai: data.body.hasConnectedOpenai
        }
    };
};
...
class Write extends React.Component<Props, State> {
    ...
    constructor(props: Props) {
        ...
        this.state = {
            ...
            hasConnectedOpenai: props.hasConnectedOpenai,
            title: '',
            url: '',
            content: '',
            description: '',
        ...
        this.authUpdateKey = authStore.subscribe((state) => {
            this.setState({
                ...
                hasConnectedOpenai: state.hasConnectedOpenai
            });
        });
    ...
    async handleClickAutoGenerate() {
        const { content } = this.state;
        if (!content) {
            snackBar('😅 내용을 작성해주세요.');
            return;
        }
        const { data } = await API.postAutoGenerateDescription({
            text_md: content
        });
        if (data.status === 'ERROR') {
            if (data.errorCode === API.ERROR.NEED_OPENAI) {
                authStore.set(state => ({
                    ...state,
                    hasConnectedOpenai: false
                }));
            }
            snackBar('😥 요약 생성에 실패했습니다.');
            return;
        }
        this.setState({
        ...
        return (
            <EditorLayout
                ...
                description={{
                    ...
                    visibleAutoGenerate: this.state.hasConnectedOpenai,
                    onClickAutoGenerate: this.handleClickAutoGenerate.bind(this)
                }}
                series={{
