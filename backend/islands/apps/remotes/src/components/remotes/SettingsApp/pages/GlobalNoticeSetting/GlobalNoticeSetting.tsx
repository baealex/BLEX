...
const GlobalNoticeSetting = () => {
    ...
    const [editingNotice, setEditingNotice] = useState<GlobalNoticeData | null>(null);
    ...
    const createMutation = useMutation({
        mutationFn: (data: GlobalNoticeCreateData) => createGlobalNotice(data),
        onSuccess: () => {
            toast.success('공지가 생성되었습니다.');
            queryClient.invalidateQueries({ queryKey: ['global-notices'] });
            closeModal();
        },
        onError: () => {
    ...
    const updateMutation = useMutation({
        mutationFn: ({ id, data }: { id: number; data: GlobalNoticeUpdateData }) => updateGlobalNotice(id, data),
        onSuccess: () => {
    ...
    const deleteMutation = useMutation({
        mutationFn: (id: number) => deleteGlobalNotice(id),
        onSuccess: () => {
    ...
    const handleEdit = (notice: GlobalNoticeData) => {
    ...
    const handleToggleActive = (notice: GlobalNoticeData) => {
        updateMutation.mutate({
            id: notice.id,
                    ...
                    <div className="p-6 space-y-6">
                        ...
                        <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
                            <Checkbox
                                ...
                                label="공지 활성화"
                                description="활성화된 공지만 사용자에게 표시됩니다."
                            />
                        </div>
                    </div>
