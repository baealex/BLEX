...
import { getBanners, createBanner, updateBanner, deleteBanner, updateBannerOrder, type BannerData, type BannerCreateData } from '~/lib/api/settings';
...
const BannerSetting = () => {
    ...
    const updateMutation = useMutation({
        mutationFn: ({ id, data }: { id: number; data: Partial<BannerData> }) => updateBanner(id, data),
        onSuccess: () => {
    ...
    const handleUpdate = (data: Partial<BannerData>) => {
        if (editingBanner) {
    ...
    return (
        <div>
            <SettingsHeader
                title="배너 관리"
                description="블로그에 표시할 배너를 관리하세요. 줄배너와 사이드배너를 위치별로 설정할 수 있습니다."
            />
            ...
            {isFormOpen && (
                <Card title={editingBanner ? '배너 수정' : '새 배너 만들기'} className="mb-6">
                    <BannerForm
            ...
            <Card title="배너 목록" className="mb-6">
                {isLoading ? (
