...
const LoadingState = ({ rows = 3, type = 'form', ariaLabel = 'Loading' }: LoadingStateProps) => {
    if (type === 'list') {
        return (
            <div className="space-y-3 animate-pulse" role="status" aria-live="polite" aria-label={ariaLabel}>
                {[...Array(rows)].map((_, i) => (
    ...
    if (type === 'spinner') {
        return (
            <div className="flex items-center justify-center p-8" role="status" aria-live="polite" aria-label={ariaLabel}>
