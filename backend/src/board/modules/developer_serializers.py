...
class DeveloperPostSerializer:
    ...
    def summary(post):
        ...
        return {
            ...
            'cover_layout': config.cover_layout if config else 'default',
            'cover_image_position': config.cover_image_position if config else 'right',
            'cover_image_ratio': config.cover_image_ratio if config else 'auto',
