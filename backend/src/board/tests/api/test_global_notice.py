...
class GlobalNoticeAPITestCase(TestCase):
    ...
    def _create_global_notice(self, **kwargs):
        defaults = dict(
    ...
    def test_get_notices_with_multiple(self):
        """여러 공지 목록 조회 테스트"""
        self._create_global_notice(title='Notice 1', url='https://example.com/1')
        self._create_global_notice(title='Notice 2', url='https://example.com/2')
    ...
    def test_normal_user_cannot_delete(self):
        ...
        self.assertTrue(SiteNotice.objects.filter(id=notice.id).exists())
