...
| P3 | `Series.save()` | Generates URL when empty and refreshes `updated_date` on every save. | Slug policy and timestamp mutation are hidden in model save; bulk updates or partial updates can bypass/override expectations. | Characterize URL generation/collision and timestamp refresh; consider extracting URL generation to series mutation service while keeping a compatibility guard. |
...
9. `Series.save()` slug/timestamp characterization before moving slug policy into series mutation service.
